...
(function($) {
  ...
  const NavigationBuilder = {
    ...
    getFormData: function() {
      const mobileEnabled = this.mobileToggle.is(':checked');
      ...
      if (mobileEnabled) {
        data.mobile_breakpoint = parseInt(this.breakpointInput.val());
        data.hamburger_animation = $('input[name="hamburger_animation"]:checked').val();
        data.menu_position = $('input[name="menu_position"]:checked').val();
        data.submenu_behavior = $('input[name="submenu_behavior"]:checked').val();
        data.submenu_depth_mobile = parseInt($('#submenu-depth-mobile').val()) || 1;
        data.close_methods = closeMethods;
      }
    ...
    /**
     * Handle mobile menu support toggle
     */
    handleMobileToggle: function() {
      const isEnabled = this.mobileToggle.is(':checked');

      if (isEnabled) {
        this.mobileSettings.slideDown(200);
        $('.etch-mobile-only-setting').slideDown(200);
      } else {
        this.mobileSettings.slideUp(200);
        $('.etch-mobile-only-setting').slideUp(200);
      }

      // Show/hide JS tab and its content
      if (isEnabled) {
        $('.etch-tab--js').show();
        $('.etch-tab-content--js').css('display', '');
      } else {
        $('.etch-tab--js').hide();
        $('.etch-tab-content--js').hide();

        // If JS tab was active, switch to JSON tab
        if ($('.etch-tab--js').hasClass('is-active')) {
          $('.etch-tab').removeClass('is-active');
          $('.etch-tab-content').removeClass('is-active');
          $('.etch-tab[data-tab="etch-json"]').addClass('is-active');
          $('.etch-tab-content[data-content="etch-json"]').addClass('is-active');
        }
      }
    },
    ...
    displayGeneratedCode: function(data) {
      ...
      if (mobileEnabled && data.javascript) {
    ...
    copyCode: function(e) {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(code).then(function() {
          ...
          NavigationBuilder.fallbackCopy(code, $button);
        });
      } else {
     ...
     */
    fallbackCopy: function(code, $button) {
      const $temp = $('<textarea>');
      $('body').append($temp);
